...
namespace TempControl
{
    public partial class FormInst : Form
    {
        ...
        public FormInst(DeviceStateM dev)
        {
            ...
            _device.instDevice.InstDeviceInitedEvent += _device_InstDeviceInitedEvent;
        }
        ...
        private void loadInstInfo()
        {
            ...
            for (int idx = 0; idx < _device.instDevice._instPorts.Count; idx++)
            {
                try
                {
                    JObject chd = (JObject)_device.instDevice._instPorts[idx];
            ...
            textBox_testID.Text = _device.instDevice._instDevices.Count <= 1 ? "null" : _device.instDevice._instDevices[1].GetBasicInfo().testId;

            this.Text = _device.instDevice._instDevices.Count <= 1 ? "null" : _device.instDevice._instDevices[1].GetBasicInfo().testId;
        ...
        private void checkBox_set2dev_Click(object sender, EventArgs e)
        {
            ...
            foreach (var itm in instList)
            {
                ...
                {
                    confOK &= _device.instDevice._instDevices[itm.Id].DisableInstDevice();
                }
                else
                {
                    _device.instDevice._instDevices[itm.Id].GetBasicInfo().PortName = portList[itm.portId].portName;
                    _device.instDevice._instDevices[itm.Id].GetBasicInfo().BaudRate = baudList[itm.brId].baudRate;
                    confOK &= _device.instDevice._instDevices[itm.Id].InitWithInfo();
