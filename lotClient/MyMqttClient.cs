...
namespace LotClient
{
    public class MyMqttClient
    {
        ...
        /// </summary>
        /// <returns></returns>
        public bool Initialize(string configFile, params SubTopic[] topics)
        {
            // 清空原有主题
            topicsSubs.Clear();
            ...
            if (mqttClient == null)
            {
                ...
                {
                    // 首先读取 mqtt 连接信息
                    System.IO.StreamReader file = System.IO.File.OpenText(configFile);
                    JsonTextReader reader = new JsonTextReader(file);
                    mqttConfig = (JObject)JToken.ReadFrom(reader);
                    file.Close();
